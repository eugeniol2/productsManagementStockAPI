...
- Nunca crie um commit sem perguntar antes. Sugira o commit só quando uma unidade de lógica estiver completa na sua forma mínima — uma rota nova, uma extração de constantes, uma correção. Uma unidade por commit, com o prefixo que a descreve (`feat`, `refactor`, `fix`, `docs`, `chore`). Não agrupe mudanças sem relação nem proponha commit de trabalho pela metade.
